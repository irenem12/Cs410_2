...
int processCommand(int currentCommand);
...
int doPipe(char *argumentsBuffer, int isBackground);
...
int processCommand(int currentCommand) {
  ...
  return doPipe(argumentsBuffer, isBackground);


}

// recursivly do piple while couting number of commands to skip
int doPipe(char *argumentsBuffer, int isBackground) {
  ...
  if (currentCommand == numCommands - 1 || strcmp(separators[currentCommand], "|") != 0) {
    ...
    if (cpid == 0) {
      ...
      currentCommand++;
      int step = processCommand(currentCommand);
...
int main(int argc, char *argv[], char *envp[])
{
  ...
  while (1) {
    ...
    while (currentCommand < numCommands) {
      // process and execute a commands or multiple commands in chain due to pipe
      // for example, 1 command was executed, skip = 1, we will skip to the next one
      // if we processed 3 commands cmd1 | cmd2 | cmd3,skip would be 3
      int skip = processCommand(currentCommand);
